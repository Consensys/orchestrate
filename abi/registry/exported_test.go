...
func TestInit(t *testing.T) {
	...
	res, err := GlobalRegistry().GetContractABI(&abi.Contract{Name: "ERC20", Tag: "v0.1.3"})
	assert.NotNil(t, res, "Method should be available")
	assert.NoError(t, err, "Should not error")

	res, err = GlobalRegistry().GetContractBytecode(&abi.Contract{Name: "ERC20", Tag: "v0.1.3"})
	assert.NotNil(t, res, "Bytecode should be available")
	assert.NoError(t, err, "Should not error")

	res, err = GlobalRegistry().GetContractDeployedBytecode(&abi.Contract{Name: "ERC20", Tag: "v0.1.3"})

...
func TestSplitTenant(t *testing.T) {
	...
	tests := []struct {
		...
		{
			"pkey with tenant",
			"b49ee1bc-f0fa-430d-89b2-a4fd0dc98906@56202652FDFFD802B7252A456DBD8F3ECC0352BBDE76C23B40AFE8AEBD714E2E",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				if err.Error() != "42300@: The key have more than one separator as @" {
					t.Errorf("SplitTenant() error = %v, wantErr %v", err, tt.wantErr)
					return
				}
			}

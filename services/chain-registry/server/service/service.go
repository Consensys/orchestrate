...
func NewManager(configs map[string]*runtime.ServiceInfo, defaultRoundTripper http.RoundTripper, metricsRegistry metrics.Registry, routinePool *safe.Pool, api, rest http.Handler) *Manager {
	return &Manager{
...
func (m *Manager) BuildHTTP(rootCtx context.Context, serviceName string, responseModifier func(*http.Response) error) (http.Handler, error) {
	...
	serviceName = utils.GetQualifiedName(ctx, serviceName)
	ctx = utils.AddProviderInContext(ctx, serviceName)
	...
	case conf.Mirroring != nil:
		var err error
		lb, err = m.getLoadBalancerMirrorServiceHandler(ctx, conf.Mirroring, responseModifier)
		if err != nil {
			conf.AddError(err, true)
			return nil, err
		}
...
func (m *Manager) getLoadBalancerMirrorServiceHandler(ctx context.Context, config *dynamic.Mirroring, responseModifier func(*http.Response) error) (http.Handler, error) {

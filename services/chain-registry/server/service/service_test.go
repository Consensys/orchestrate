...
func TestGetLoadBalancerServiceHandler(t *testing.T) {
	...
	serverAuth := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		assert.Equal(
			t,
			fmt.Sprintf("Basic %v", basicAuth("test-user", "test-pwd")),
			r.Header.Get("Authorization"),
		)
	}))
	defer serverAuth.Close()
	authURL, _ := url.Parse(serverAuth.URL)
	...
	testCases := []struct {
		...
		{
			desc:        "Basic Auth set correctly",
			serviceName: "test",
			service: &dynamic.ServersLoadBalancer{
				Sticky: &dynamic.Sticky{Cookie: &dynamic.Cookie{}},
				Servers: []dynamic.Server{
					{
						URL: fmt.Sprintf("http://test-user:test-pwd@%v", authURL.Host),
					},
				},
			},
			expected: []ExpectedResult{
	...
	for _, test := range testCases {
		...
		t.Run(test.desc, func(t *testing.T) {
			...
			if len(test.providerName) > 0 {
				ctx = utils.AddProviderInContext(ctx, "foobar@"+test.providerName)
			}

...
func TestHook(t *testing.T) {
	...
	store.EXPECT().Store(gomock.Any(), gomock.Any(), gomock.Any())
	...
	testEnvelope := tx.NewEnvelope().
		SetID("dce80ed3-8b0e-4045-9a91-832ba0391c44").
		SetChainID(big.NewInt(1)).
		SetTxHash(*receipt.GetTxHashPtr())
	store.EXPECT().LoadByTxHash(gomock.Any(), gomock.Any()).Return(&svc.StoreResponse{
		StatusInfo: &svc.StatusInfo{Status: svc.Status_PENDING},
		Envelope:   testEnvelope.TxEnvelopeAsRequest(),
	}, nil).AnyTimes()
	producer.ExpectSendMessageAndSucceed()
	err = hk.AfterNewBlock(context.Background(), c, &block, []*types.Receipt{receipt})
	assert.NoError(t, err, "#1 AfterNewBlock should not error")

	// Test 2: we store envelope on envelope store
	_, err = store.Store(context.Background(), &svc.StoreRequest{
		Envelope: tx.NewEnvelope().SetID(uuid.NewV4().String()).SetChainID(c.ChainID).SetTxHash(ethcommon.HexToHash(receipt.TxHash)).TxEnvelopeAsRequest(),
	})
	assert.NoError(t, err)
	producer.ExpectSendMessageAndSucceed()
	err = hk.AfterNewBlock(context.Background(), c, &block, []*types.Receipt{receipt})

...
func TestHook(t *testing.T) {
	...
	envelope := &tx.Envelope{
		Tx: tx.Tx{
			TxHash: &(&struct{ x ethcommon.Hash }{ethcommon.HexToHash("0xf2beaddb2dc4e4c9055148a808365edbadd5f418c31631dcba9ad99af34ae66b")}).x,
		},
	...
	err = hk.AfterNewBlock(context.Background(), c, &block, []*tx.Envelope{envelope})
	assert.NoError(t, err, "#1 AfterNewBlock should not error")

	// Test 2: producer fails
	producer.ExpectSendMessageAndFail(fmt.Errorf("test-producer-error"))
	err = hk.AfterNewBlock(context.Background(), c, &block, []*tx.Envelope{envelope})
	assert.Error(t, err, "#2 AfterNewBlock should error")

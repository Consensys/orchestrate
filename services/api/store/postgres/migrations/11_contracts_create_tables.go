...
func createContextContractTable(db migrations.DB) error {
	log.Debug("Creating contract tables...")
	_, err := db.Exec(`
...
CREATE INDEX ON events (sig_hash, indexed_input_count, codehash);
	`)
	if err != nil {
		log.WithError(err).Error("Could not create contract tables")
		return err
	}
	log.Info("Created contract tables")

	return nil
}

func dropContextContractTable(db migrations.DB) error {
	log.Debug("Dropping contract tables")
	_, err := db.Exec(`
DROP TABLE events;
DROP TABLE methods;
...
`)
	...
	log.Info("Dropped contract tables")

	return nil
}

func init() {
	Collection.MustRegisterTx(createContextContractTable, dropContextContractTable)
}

...
func (app *App) Run() {
	...
	// Consume from mutltiple topics (one per chain)
	topics := []string{}
	for _, chainID := range app.infra.Mec.Networks(context.Background()) {
		topics = append(topics, fmt.Sprintf("%v-%v", viper.GetString("worker.in"), chainID.Text(16)))
	}

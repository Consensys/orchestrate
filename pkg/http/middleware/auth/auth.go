...
func (a *Auth) Handler(h http.Handler) http.Handler {
	return http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) {
		if a.multitenancy {
			...
			apiKeyCtx := authutils.WithAPIKey(
				req.Context(),
				req.Header.Get(authutils.APIKeyHeader),
			)

			// Perform API Key Authentication
			checkedCtx, err := a.key.Check(apiKeyCtx)
			if err == nil {
				// Bypass JWT authentication
				log.FromContext(checkedCtx).Debugf("authentication succeeded (API-Key)")
			...
			if err == nil {
				...
				log.FromContext(checkedCtx).
					WithField("tenant_id", jwtTenantID).
